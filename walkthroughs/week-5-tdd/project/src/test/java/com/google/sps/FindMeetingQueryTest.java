...
public final class FindMeetingQueryTest {
  ...
  public void justEnoughRoomWithOptional() {
    // Have one person, but make it so that there is just enough room at one point in the day to
    // have the meeting. Add optional person who's time is that one place.
    //
    // Events  : |--A--|     |----A----|
    // Day     : |---------------------|
    // Options :       |-----|
  ...
  public void onlyOptionalAttendees() {
    // Only optional attendees are added, should return the three open time slots.
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
  ...
  @Test
  public void onlyOptionalAttendeesNoFreeTime() {
  ...
  public void paritalOptionalAttendees() {
    ...
    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected =
        Arrays.asList(TimeRange.fromStartEnd(TIME_1000AM, TimeRange.END_OF_DAY, false));
